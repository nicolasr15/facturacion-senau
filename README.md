...
**Fase 3 — Integración. Hecho.** `senau-tickets` llama a este Worker por service binding (`[[services]] binding = "FACTURACION"`) desde `src/lib/payments.ts`, tanto al aprobar el webhook de Bold como al conciliar; `src/lib/facturacion.ts` arma la petición y nunca deja que un fallo de este Worker afecte la compra (ver pruebas en `senau-tickets/test/run.ts`). El checkout de `senau-tickets` ya tiene un selector real de tipo de documento (CC/CE/TI/PP, `src/lib/doc-types.ts`), así que `id_type`/`id_number` ya no se infieren de un heurístico — se traducen directo a los códigos DIAN (ver `idTypeFor()` en `senau-tickets/src/lib/facturacion.ts`).
